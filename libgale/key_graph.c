...
struct graph {
	...
	int flags;
	void *(*func)(oop_source *,struct gale_map *,int,void *);
	void *user;
...
static void *found(oop_source *oop,struct gale_key *key,void *user) {
	struct graph *g = (struct graph *) user;
	struct gale_group data = gale_key_data(gale_key_public(key,g->now));
	...
	return g->func(oop,g->map,g->has_null,g->user);
}

void key_i_graph(oop_source *oop,
	struct gale_key *root,int flags,struct gale_text name,
	void *(*func)(oop_source *,struct gale_map *,int,void *),void *user)
{
	struct graph *graph;
	gale_create(graph);
	graph->name = name;
