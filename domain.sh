...
if gkinfo "$CONF_GALE_DOMAIN" 2>/dev/null | grep -q 'Signed by: <ROOT>' ; then
...
EOM
	...
	if gkinfo < "$skey" | grep -q "^Public key: <$CONF_GALE_DOMAIN>" ; then
	...
	if gkinfo < "$skey" | grep -q "Signed by: <ROOT>" ; then
